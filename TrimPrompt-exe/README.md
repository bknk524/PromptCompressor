...
圧縮精度の最新結果と、アプリ内判定・Codex意味差分判定の方法は、先に次のファイルを確認してください。

```text
資料/評価メモ/2026-07-22_Codex意味評価反復改善レポート.md
```

従来の評価条件、個別・全件評価コマンド、作業引き継ぎは `資料/評価メモ/Codex引き継ぎ_レベル2圧縮改善.md` に残しています。
...
- LLM出力は必須語、数値、識別子、否定、対象限定、状態維持、方法を限定する条件、処理順序、失敗・タイムアウト時の挙動、検証条件を確認し、不正な場合は原文を返す
- 前処理では明示的な訂正や意味を持たない断片だけを整理する。後処理では安全な文末整形と、欠落した技術用語・条件の決定的な復元を行い、復元候補を最終検証してから採用する
- 通常の圧縮経路では修復用の2回目のLLM推論を行わない。追加処理は制約抽出、文字列照合、決定的な復元に限り、圧縮時間への影響を抑える
- 原文返しになった場合は、ランタイム失敗、確認できなかった要件、数値、識別子、否定・禁止・維持条件などの理由を結果欄へ表示する。原文の自動コピーは行わない
...
- `cargo test --workspace --no-default-features -q`: 241件成功、1件を除外、失敗0件
...
- 新規の高密度11ケース: アプリ内判定11/11、Codex意味差分判定11/11、原文返し0件
- 既存レベル2基準30件: 30/30、平均文字数比78.11%、原文返し0件
- 長文CSVケース: 合格、文字数比57.58%、原文返し0件
- compatible、AVX2、AVX-512の3種類について、Rustとllama.cppの命令セットを一致させた厳格リリースビルド: 成功
- パッケージ内の実モデルによる固定品質テスト: 各CPU版5/5、合計15/15成功
...
- 現行fixtureに含まれない言語表現、専門領域、極端に長い入力をすべて保証するものではない
- AVX-512版はこのPCで実モデル実行まで確認済みだが、他の対応CPUでも初回診断の実測結果と要件充足結果に基づいてAVX2版と比較する
...
- `codex-semantic-regression-v1.json`: 方法制約、処理順序、失敗時挙動、冪等性、DSTなどを含む高密度11件
...
```text
application/resources/evaluations/codex-semantic-regression-v1.json
...
```powershell
powershell -ExecutionPolicy Bypass -File application/tools/evaluate-prompt-profiles.ps1 -FixturePath application/resources/evaluations/codex-semantic-regression-v1.json -Levels 2
```
...
2026-07-22の実LLM最終評価では、現行のモデル、プロンプト、前処理、後処理、検証器でレベル2基準30件が全件合格しました。新規高密度11件と長文CSVケースを合わせた全42件で原文返しは0件です。高密度11件はCodex側でも原文と最終出力を比較し、条件欠落、条件反転、処理順序、例外時挙動、意味の追加、文章の自然さを確認して11/11合格と判定しました。圧縮プロンプト、前処理、後処理、検証、fixture、モデル、組み込みランタイムのいずれかを変更した場合は、関連ケースを個別確認した後、必ず全30件と高密度11件を再実行します。
