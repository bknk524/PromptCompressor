...
fn configure_windows_cmake() {
    if !cfg!(windows) || env::var_os("CMAKE").is_some() || command_exists("cmake") {
        return;
    }
    ...
    for version in ["18", "2022"] {
        for edition in ["Community", "BuildTools", "Professional", "Enterprise"] {
            let cmake = visual_studio_root
                .join(version)
                .join(edition)
                .join("Common7/IDE/CommonExtensions/Microsoft/CMake/CMake/bin/cmake.exe");
            if cmake.is_file() {
                // cmake crateはCMAKE環境変数を実行ファイルの明示指定として扱う。
                unsafe { env::set_var("CMAKE", &cmake) };
                debug_log!("Using Visual Studio CMake at {}", cmake.display());
                return;
            }
        }
    }
...
fn main() {
    ...
    if cfg!(feature = "mtmd") {
        ...
        // llama-cpp-4 exposes fit, memory and MTP shims even when those APIs are
        // unused by the application, so their common symbols must remain linkable.
        config.define("LLAMA_BUILD_COMMON", "ON");
    }
    ...
    let want_avx2 = cfg!(feature = "avx2") || cfg!(feature = "avx512");
    let want_avx512 = cfg!(feature = "avx512");
    if is_x86_target && !want_native {
        // SSE4.2 is the effective baseline for x86_64; leave it ON.
        config.define("GGML_SSE42", "ON");
        // CPU engine features are explicit so distributable builds never inherit the build host.
        config.define("GGML_AVX", if want_avx2 { "ON" } else { "OFF" });
        config.define("GGML_AVX2", if want_avx2 { "ON" } else { "OFF" });
        config.define("GGML_AVX_VNNI", "OFF");
        config.define("GGML_FMA", if want_avx2 { "ON" } else { "OFF" });
        config.define("GGML_F16C", if want_avx2 { "ON" } else { "OFF" });
        config.define("GGML_BMI2", if want_avx2 { "ON" } else { "OFF" });
        config.define("GGML_AVX512", if want_avx512 { "ON" } else { "OFF" });
        ...
        if cache.exists() {
            ...
            if !has_makefile && !has_ninja {
                ...
                let x86_isa_mismatch = is_x86_target_local && !want_native && {
                    let option_matches = |name: &str, enabled: bool| {
                        let expected = format!(
                            "{name}:BOOL={}",
                            if enabled { "ON" } else { "OFF" }
                        );
                        cache_contents.contains(&expected)
                    };
                    !option_matches("GGML_AVX", want_avx2)
                        || !option_matches("GGML_AVX2", want_avx2)
                        || !option_matches("GGML_FMA", want_avx2)
                        || !option_matches("GGML_F16C", want_avx2)
                        || !option_matches("GGML_BMI2", want_avx2)
                        || !option_matches("GGML_AVX512", want_avx512)
                };
                ...
                // llama-cpp-4 always references common helper symbols from its
                // compiled modules, even when the application does not call them.
                let common_mismatch =
                    !cache_contents.contains("LLAMA_BUILD_COMMON:BOOL=ON");
    ...
    if !build_shared_libs {
        ...
        let common_profile_dir = common_build_dir.join(&profile);
        if common_build_dir.exists() {
            println!(
            ...
            for library in ["llama-common", "llama-common-base"] {
                if !llama_libs.iter().any(|linked| linked == library) {
                    println!("cargo:rustc-link-lib=static={library}");
                }
            }
        }

        let httplib_build_dir = cmake_out_dir
            .join("build")
            .join("vendor")
            .join("cpp-httplib");
        if httplib_build_dir.exists() {
            println!(
                "cargo:rustc-link-search=native={}",
                httplib_build_dir.display()
            );
            println!(
                "cargo:rustc-link-search=native={}",
                httplib_build_dir.join(&profile).display()
            );
