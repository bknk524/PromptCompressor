...
```powershell
prompt-compressor "React一覧画面の検索を修正したい"
prompt-compressor --file prompt.txt --level 2 --profile standard
Get-Content .\prompt.txt | prompt-compressor --format json
prompt-compressor compare --file prompt.txt --profiles standard,code_focused
```
