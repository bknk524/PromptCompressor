...
- `TrimPrompt.exe` は起動直後にCPU命令セットをOS込みで判定する。AVX2、FMA、F16Cを満たせば安全な初期値としてAVX2版を選び、満たさない場合は互換版へ戻す。AVX-512版はAVX512F、CD、BW、DQ、VLをすべて満たすCPUだけを候補にし、初回から無条件には使わない
- モデル取得後の初回起動では通常UIをまだ表示せず、リングと「初期設定中」の文言だけを表示してCPU診断を行う。CPUスレッド数は実モデルで入力の一括評価と逐次生成を交互に3回ずつ計測し、ばらつきが20%を超えた候補は2回追加測定する。中央値から選んだ結果をCPU命令セット、推論エンジン、CPU、モデル、実行設定ごとに `application/local/state/inference-tuning-v1/` へ保存する
- AVX-512対応CPUでは、スレッド測定後にAVX2版とAVX-512版を5種類の固定入力による実圧縮パイプラインで比較する。モデル読込時間を除き、前処理、入力準備、LLM生成、検証までの平均時間を測り、AVX-512版が3%以上速い場合だけ次回起動から採用する。同等、遅い、測定失敗の場合はAVX2版を維持する
- 診断結果を保存したらランチャー経由で自動再起動し、選択した設定を読み込む。AVX-512版が選ばれた場合は、再起動後にAVX-512版用のスレッド診断を行ってもう一度だけ自動再起動する。以後は保存設定が有効な限り診断画面を出さず、最適化済み設定で通常UIを起動する
- 診断に失敗した場合は未最適化の通常UIへ進まず、初期設定画面にエラーを表示する。次回起動時に保存状態を再確認し、未完了の診断を再試行する
- CPUエンジンの選択結果はCPU識別情報とパッケージのビルドIDに結び付ける。CPU、モデル、診断方式、またはビルドが変われば初期設定画面で再測定し、選択した内部EXEが欠けている場合はAVX2版または互換版へ戻す。設定画面の「CPU最適化」から手動で再調整した場合も同じ初期設定画面と自動再起動を使う
...
- 入力停止から1.2秒後に、現在の原文と設定に対応する生成直前のモデル内部状態を1件だけ先読みする。圧縮時はセッションを複製せず、そのまま1回だけ生成処理へ引き渡す。圧縮結果は保存せず、同じ入力の再実行でも毎回LLMの生成処理を行う
...
2026-07-18時点の最新ソースとリリースEXEで、次を確認済みです。

- `cargo fmt --all -- --check`: 成功
- `cargo metadata --locked --no-deps`: 成功
- `cargo test --workspace`: 195件成功、Hugging Face通信を伴う1件を除外
...
- llama.cpp `b9982` 採用後の同一条件による旧版との速度比較、固定5入力の出力比較、レベル2基準30件の全件評価は未実施
- AVX-512版は対応CPU上での実行確認が必要。対応CPUであっても無条件に採用せず、初回診断の実測結果でAVX2版と比較する
...
推論ランタイムはCPU別の内部EXEへ静的に組み込みます。ユーザーが外部ランタイムを導入したり、CPU版を手動で選んだりする必要はありません。

組み込みランタイムは `llama-cpp-4 0.4.2` と llama.cpp `b9982` (`99f3dc32296f825fec94f202da1e9fede1e78cf9`) です。`native` ビルドには依存せず、次の命令セットを明示して別々に構築します。

- compatible: SSE4.2基準、AVX系は無効
- AVX2: AVX、AVX2、FMA、F16C、BMI2
- AVX-512: AVX2構成にAVX-512基本セットを追加。VBMI、VNNI、BF16は無効

各内部EXEはアプリ側のfeatureとllama.cpp側の命令セットが一致するかモデル読込時に自己検査します。起動ランチャーはCPU命令セットを判定し、対応する内部EXEだけを起動します。
